  ...
  try {
    ...
    responseNextPage.rows = responseNextPage.rows.map((row) => {
      if (row.date_have_been_done) {
        row.date_have_been_done = moment(row.date_have_been_done).format(
          "YYYY-MM-DD"
        );
      }
      return row;
...
router.post("/", authorization, async (req, res) => {
  ...
  INSERT INTO projects (
    ...
    "teamleader", "fullstackdeveloper", "frontenddeveloper", "backenddeveloper", "designer",
  ...
  pool.query(
    ...
    [
      ...
      newProject.fullstackdeveloper,
      newProject.frontenddeveloper,
      newProject.backenddeveloper,
      newProject.designer,
